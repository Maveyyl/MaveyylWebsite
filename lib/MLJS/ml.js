...
var ml = (function(){ 
	...
	GradientDescentRegressionContext.prototype.check_gradient = function (X, y, theta, epsilon, cost_function){
		...
		for(n=0;n<this.N;n++){
			...
			if (  
				!(cost.grad[n] === num_grad && num_grad === 0) &&
				Math.abs(cost.grad[n] - num_grad) / (Math.abs(cost.grad[n]) + Math.abs(num_grad)) > 2* epsilon
			)
				return false;
		}
	...
	ml.NN = ml.NN || {};
	ml.NN.utils = neuralNetworkUtils;
	...
	ml.NN = ml.NN || {};
	ml.NN.FullyConnectedLayer = FullyConnectedLayer;
	...
	function FullyConnectedLayer( input_unit_count, output_unit_count, weight_init_f, activation_f, activation_f_derivative ){
		...
		for(o=0;o<this.output_unit_count;o++){
			...
			for(i=0;i<this.input_unit_count;i++){
				this.weights[o][i] = weight_init_f();
			}
		}
	}
	FullyConnectedLayer.prototype.forward_propagation = function( x ){
		...
		z = new Array(this.output_unit_count).fill(0);
		a = new Array(this.output_unit_count);
	
		for(o=0;o<this.output_unit_count;o++){
			for(i=0;i<this.input_unit_count;i++){
				z[o] += x[i] * this.weights[o][i];
			}
			a[o] = this.activation_f( z[o] );
		}
	
		// console.log("x", x, "a", a, "z", z, "weights", this.weights);
		return { a: a, z: z} ;
	};
	FullyConnectedLayer.prototype.backward_propagation = function( d, z ){
		var i,o,op;
	
	
		var dm = new Array(this.input_unit_count-1).fill(0);
	
		for(i=1;i<this.input_unit_count;i++){
			for(o=0;o<this.output_unit_count;o++)
				dm[i-1] += d[o] * this.weights[o][i];
			
			dm[i-1] *= this.activation_f_derivative(z[i-1]);
		}
	
		return dm;
	};
	FullyConnectedLayer.prototype.last_layer_gradient_compute = function( a, z, y){
		var o;
	
		var d = new Array(a.length);
		for(o=0;o<this.output_unit_count;o++)
			d[o] = (a[o] - y[o]) * this.activation_f_derivative( z[o] );
	...
	FullyConnectedLayer.prototype.weights_update = function( gradients, alpha ){
		...
		for(i=0;i<this.input_unit_count;i++){
			for(o=0;o<this.output_unit_count;o++){
				this.weights[o][i] -= alpha * gradients[o][i];
			}
		}
	};
	...
	ml.NN = ml.NN || {};
	ml.NN.NeuralNetwork = NeuralNetwork;
	
	function NeuralNetwork( cost_f ){
		this.cost_f = cost_f; 
		this.layers = [];
		this.layer_count = 0;
	}
	...
	NeuralNetwork.prototype.predict = function( x ){
		...
		var y = this.forward_propagation( x ).a[L];
	...
	NeuralNetwork.prototype.compute_cost = function( X, Y, lambda){
		...
		var H = new Array(M); // will contain the hypothesis for each example, MxO
		var A = new Array(M); // will contain the activation value of each unit for each example, MxL+1xO
		var Z = new Array(M); // will contain the afines value of each unit for each example, MxLxO
		var D = new Array(M); // will contain the propagated error for each unit for each example MxLxO
	
		for(m=0;m<M;m++){
			r = this.forward_propagation( X[m] );
			A[m] = r.a;
			Z[m] = r.z;
	
			l = A[m].length-1; // last layer index
			O = A[m][l].length; // number of units in last layer
			H[m] = A[m][l]; // copies the activation value of the last layer in the hypothesis array
	
			r = this.backward_propagation( A[m], Z[m], Y[m] );
			D[m] = r.d;
		}
		...
		return this.regularize(J, grad, lambda, M);
	};
	NeuralNetwork.prototype.compute_gradient = function(D, A){
		var grad = new Array(this.layers);
		...
		var O,I;
		for( l=0;l<L;l++){
			O = this.layers[l].output_unit_count;
			grad[l] = new Array(O);
			for( o=0;o<O;o++){
				I = this.layers[l].input_unit_count;
				grad[l][o] = new Array(I).fill(0);
				for(i=0;i<I;i++){
					for(m=0;m<M;m++)
						grad[l][o][i] += D[m][l][o] * A[m][l][i]; // it's the D of layer l+1 and A of layer l, but since A is of length l+1 the equation is written with both using l as index
					grad[l][o][i] = grad[l][o][i] / M;
				}
			}
		}
	...
	NeuralNetwork.prototype.check_gradient = function(X, Y, epsilon){
		...
		for(l=0;l<L;l++){
			...
			for(i=0;i<I;i++){
				for(o=0;o<O;o++){
					this.layers[l].weights[o][i] -= epsilon;
					cost_m = this.compute_cost( X, Y, 0).J;
	
					this.layers[l].weights[o][i] += 2*epsilon;
					...
					console.log(grad[l][o][i], num_grad);
					if (  
						!(grad[l][o][i] === num_grad && num_grad === 0) &&
						Math.abs(grad[l][o][i] - num_grad) / (Math.abs(grad[l][o][i]) + Math.abs(num_grad)) > 2* epsilon
					)
						return false;
				}
			}
	...
	NeuralNetwork.prototype.regularize = function(J, grad, lambda, M){
		...
		if( lambda !== 0 ){
			...
			for(l=0;l<L;l++){
				...
				for(o=0;o<O;o++){
					I = this.layers[l].input_unit_count;
					for(i=1;i<I;i++){ // start at 1 because we don't regularize with bias parameters
						J_reg +=  this.layers[l].weights[o][i] * this.layers[l].weights[o][i];
	
						r.grad[l][o][i] += lambda * this.layers[l].weights[o][i] / M;
					}
				}
			}
	...
	NeuralNetwork.prototype.forward_propagation = function( x ){
		...
		a[0] = x.slice(); // copy the data
		a[0].unshift(1); // adding bias unit
	
	
		for(l=0;l<L;l++){
			r= this.layers[l].forward_propagation(a[l]);
	
			// if is not last layer, add bias unit
			if( l+1 !== L )
				r.a.unshift(1);
	...
	NeuralNetwork.prototype.backward_propagation = function( a, z, y){
		...
		for(l=L-1;l>=0;l--){ // we dont propagate error for the input units
			if( l === L-1) // for the last layer we have to compute d direcly with y
				d[l] = this.layers[L-1].last_layer_gradient_compute( a[L], z[L-1], y);
			
	
			// we compute d[l-1] according to d[l] and z[l-1]
			if( l> 0 )
				d[l-1] = this.layers[l].backward_propagation( d[l], z[l-1] );
		}
	...
	ml.NN = ml.NN || {};
	ml.NN.GradientDescent = GradientDescent;
